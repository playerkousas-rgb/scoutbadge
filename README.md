...
## v5.3.0 升級（團長唯一＋領袖免 YMIS）

1. 將最新 `apps-script/Code.gs` 貼入 Apps Script → 重新部署「新版本」（URL 不變）。無新工作表、無需執行 `initializeSheets()`。
2. 團長全團只可有一位：開戶／轉角色時若已有現任團長會被拒絕並顯示現任姓名；換人流程：先將現任轉為其他角色（或停用），再升新人。
3. 領袖免 YMIS：申請表領袖身份不再設 YMIS 欄；批准／批量開戶時後端自動編配內部編號，領袖一律用電郵＋密碼登入（帳戶交收只須轉交電郵＋臨時密碼）。
4. 批量開戶 CSV／JSON：領袖列可留空 YMIS（須填姓名＋電郵＋角色＋臨時密碼）；失敗原因會直接彈出顯示，不再只寫 console。
5. 開戶權限收緊：只可開立自己等級可管理的角色（例如支部領袖不可再開出管理員）。

## v5.2.1 升級（對齊 VSBADGE v8.2：領袖自助申請）

1. 將最新 `apps-script/Code.gs` 貼入 Apps Script → 重新部署「新版本」（URL 不變）。無新工作表、無需執行 `initializeSheets()`。
2. 登入頁「🆕 申請帳戶（成員／領袖）」可選身份：成員填 YMIS＋姓名；領袖填姓名＋電郵（v5.3.0 起毋須 YMIS，用電郵登入）。
