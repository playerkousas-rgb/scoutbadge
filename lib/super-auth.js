...
// One-time verifier bootstrap. Only the proxy holds the per-troop API key, so
// only it can mint this; it does so just after the central password matched.
// GAS verifies the signature locally without needing the callback that this
// very step is about to configure, which is what removes the chicken-and-egg:
// the SUPER_KEY holder can open a troop's central login without first having a
// leader session in that troop.
const BOOTSTRAP_TTL_MS = 5 * 60 * 1000;

function createCentralBootstrap({ verifyUrl, troopId, apikey }) {
  if (typeof apikey !== 'string' || !apikey) return '';
  const exp = Math.floor((Date.now() + BOOTSTRAP_TTL_MS) / 1000);
  const sig = crypto.createHmac('sha256', apikey)
    .update(`${String(verifyUrl)}|${String(troopId)}|${exp}`, 'utf8')
    .digest('hex');
  return `${exp}.${sig}`;
}
