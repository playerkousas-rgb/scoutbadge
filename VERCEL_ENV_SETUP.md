...
| ~~`SCOUTBADGE_VERIFY_URL`~~ | **已無作用**（可刪）。中央登入驗票端點係 `apps-script/Code.gs` 嘅 `SUPER_VERIFY_URL` 常數（VS／RS 同款）；舊版自動開通機制已移除 |
...
| 每個旅團要人手設定中央登入驗證端點（只可以由領袖 session 做） | 端點係 `Code.gs` 常數，唔使設定；但每個旅團**首次**要在 Apps Script 執行一次 `testTrustedTicketVerifier` 授權 `script.external_request` |
...
- 見到 → 旅團登記冇問題，問題喺中央登入嗰條路（`SUPER_KEY`、部署版本、後端網址、`SUPER_VERIFY_URL` 常數）。
