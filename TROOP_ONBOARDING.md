...
## F. 中央登入（系統管理員帳號）一次性設定

系統管理員（super_admin）的密碼存在 Vercel 的 `SUPER_KEY`，不存入旅團 Sheet。首次啟用前，該旅團的 Apps Script 需要設定一次「驗證端點」，讓它知道要向哪個固定 URL 回調驗證 Proxy 簽發的短效票據：

1. 以領袖（童軍團長或以上）登入系統。
2. 進入「成員管理」頁 →「🔑 中央登入設定」。
3. 端點預填為本部署域名 + `/api/verify-super-ticket`，旅團編號預填為目前旅團；確認後按「儲存設定」。
4. 系統會接著自動「測試連線」；顯示「連線測試成功」即表示回調已通、中央登入啟用。
5. 之後在登入頁以中央帳號 + Vercel 的 `SUPER_KEY` 登入即可。
