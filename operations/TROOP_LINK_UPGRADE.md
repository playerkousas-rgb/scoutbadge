...
4. **匯入零失敗＝自動刪檔**：`importUsersFromDrive()` 會即刻把匯出檔（含 hash）移入 Drive 垃圾桶（可還原 30 天），
   「操作紀錄」只記檔案 ID（唔記 hash）；有失敗筆數就**保留檔案**等修正後再匯（匯入係冪等，唔會開重複帳戶），
   彈窗會講明係「已刪」定「保留」；自動刪除失敗（權限／檔案已搬走）亦會回報，請人手刪。
   唔好用共享資料夾或電郵明文傳送個檔。
